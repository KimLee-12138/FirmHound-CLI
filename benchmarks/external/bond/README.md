...
> 工具定位：约束导向定向 fuzz，**把 KLEE 保活候选变成可触发 PoC**（CONSTRAINED_VALIDATION 阶段）
...
| H6 | KLEE 保活候选 → BOND 验证闭环 | ✅ Full（接口连通，单测模拟） |
| H7 | SKILL + 文档 | ✅ Full |
...
- `backend=wsl` 或 `docker`（镜像 `bond/mini:latest`，内置 boofuzz + ghidra-headless）。
...
3. **不伪造触发**：仿真器缺席时 `triggered=None`，宁可漏报不误报。
